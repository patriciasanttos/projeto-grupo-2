...
export default class SpreadsheetRepository {
    async saveCompany ({ data, dimensions: companyDimensions }: CompanyType, machines: { autoclaves: string, thermoWashers: string }) {
        ...
        await axios
            .post(String(process.env.ALL_COMPANYS_SPREADSHEET_URL), JSON.stringify(dataPost), {
                headers: {
    ...
    async saveQualifiedLeads ({ name, companyName, cnpj, rate }: { name: string, companyName: string, cnpj: string, rate: string }) {
        const dataPost = {
            'Cliente': name,
            'Empresa': companyName,
        ...
        await axios
            .post(String(process.env.QUALIFIED_LEADS_SPREADSHEET_URL), JSON.stringify(dataPost), {
                headers: {
                    'Content-Type': 'application/json',
                },
                params: {
